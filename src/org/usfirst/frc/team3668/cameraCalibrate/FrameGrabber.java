...
public class FrameGrabber {
	...
	public static VideoCapture open(String url, int devId, int width, int height) {
		...
		if (url != null) {
			...
			if (vc.isOpened()) {
				System.err.println("FrameGrabber - Starting IP camera feed from: " + url);
			} else {
				System.err.println("FrameGrabber - Failed to start IP camera feed from: " + url);
				vc = null;
			}
		} else {
			vc.open(devId);
			if (vc.isOpened()) {
				if (width > 0 && height > 0) {
					vc.set(Videoio.CV_CAP_PROP_FRAME_WIDTH, width);
					vc.set(Videoio.CV_CAP_PROP_FRAME_HEIGHT, height);
				}

				System.err.println("FrameGrabber - Starting Video Feed using Web Cam: " + devId);
			} else {
				System.err.println("FrameGrabber - Failed to start Video Feed using Web Cam: " + devId);
				vc = null;
			}
		}
		return vc;
